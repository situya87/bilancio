"""Export writers (balances CSV, events JSONL, HTML report) for v2 run results."""
...
def write_balances_csv(result: Any, path: Path) -> None:
    """Write balances using the same CSV shape as the existing exports."""
...
def write_events_jsonl(result: Any, path: Path) -> None:
    """Write events as JSONL, preserving Decimal values like the existing exports."""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def write_html_report(
    ...
    *,
    ...
    """Write a readable HTML report for the public ``--html`` CLI flag."""
    from bilancio_v2.views import balance_rows
