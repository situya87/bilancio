...
def write_balances_snapshot(
    system: System, path: Path, day: int, agent_ids: list[str] | None = None
) -> None:
    ...
    for agent_id in agent_ids:
        ...
        for asset_id in agent.asset_ids:
            if asset_id in system.state.contracts:
                ...
                assets.append(
                    {"id": asset_id, "type": type(contract).__name__, "amount": contract.amount}
                )
        ...
        for liability_id in agent.liability_ids:
            if liability_id in system.state.contracts:
                ...
                liabilities.append(
                    {"id": liability_id, "type": type(contract).__name__, "amount": contract.amount}
                )
