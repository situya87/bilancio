...
def show_scenario_header_renderable(
    name: str, description: str | None = None, agents: Any | None = None
) -> list[RenderableType]:
    ...
    renderables.append(Panel(header, title="Bilancio Scenario", border_style="cyan"))
        ...
        if isinstance(agents, list):
            ...
            for agent_config in agents:
                agent_id = (
                    ...
                    else str(agent_config.get("id", "unknown"))
                )
                agent_type = (
                    agent_config.kind
                    if hasattr(agent_config, "kind")
                    else str(agent_config.get("kind", "unknown"))
                )
                agent_name = (
                    agent_config.name
                    if hasattr(agent_config, "name")
                    else str(agent_config.get("name", agent_id))
                )
...
def show_scenario_header(
    name: str, description: str | None = None, agents: dict[str, Any] | None = None
) -> None:
    ...
    if day is not None:
        ...
        if system.state.events:
            ...
            if event_mode == "table":
                ...
                rep_day = (
                    0
                    if any(e.get("phase") == "setup" for e in system.state.events)
                    else system.state.day
                )
                phase_tables = display_events_tables_by_phase_renderables(
                    system.state.events, day=rep_day
                )
                renderables.extend(phase_tables)
            elif event_mode == "detailed":
                event_renderables = display_events_renderable(
                    system.state.events, format="detailed"
                )
                renderables.extend(event_renderables)
            else:
                # Summary mode
    ...
    if agent_ids is not None:
        if len(agent_ids) == 0:
            ...
            if t_account:
                ...
                balance_renderable = display_agent_balance_table_renderable(
                    system, agent_ids[0], format="rich"
                )
            ...
            if t_account:
                ...
                balance_renderable = display_multiple_agent_balances_renderable(
                    system, agent_ids, format="rich"
                )
                renderables.append(balance_renderable)
    else:
...
def show_error_panel(error: Exception, phase: str, context: dict[str, Any] | None = None) -> None:
    ...
    if isinstance(error, DefaultError):
        content.append("\n💡 ", style="yellow")
        content.append(
            "This usually means a debtor lacks sufficient funds or assets to settle an obligation.\n",
            style="dim",
        )
        content.append(
            "Check the agent's balance sheet and available means of payment.", style="dim"
        )
        ...
        content.append(
            "Common causes: duplicate IDs, negative balances, or mismatched references.",
            style="dim",
        )

    # Display the panel
    console.print(Panel(content, title=f"Error in {phase}", border_style="red", expand=False))

    # Log the error as an event
    if context and "scenario" in context:
        console.print("[dim]Error logged to simulation events[/dim]")
