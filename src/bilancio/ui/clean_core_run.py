"""CLI orchestration for clean-core scenario runs."""
...
def run_clean_core_scenario(
    *,
    ...
    from bilancio.engines.clean_core import (
        ...
        dealer_metrics_summary,
        finalize_banking_marker_events,
        prepare_scenario,
        run_runtime_until_stable,
        write_html_report,
    )
    from bilancio.engines.clean_core import (
        run_day as run_clean_day,
    )
    from bilancio.engines.clean_core import (
        write_balances_csv as write_clean_balances_csv,
    )
    from bilancio.engines.clean_core import (
        write_events_jsonl as write_clean_events_jsonl,
    )
    from bilancio.engines.clean_core_stability import update_runtime_stability
    from bilancio.engines.clean_core_types import CleanRunResult, CleanStabilityTracker
    ...
    def _check_clean_core_daily_invariants(runtime: Any, day: int) -> None:
        if check_invariants == "daily":
            invariant_checker(
                balance_rows(CleanRunResult(runtime.state, day + 1, False))
            )
        ...
        if mode == "step":
            ...
            for day in range(max_days):
                ...
                if not confirm_ask(f"[cyan]Run day {day + 1}?[/cyan]", default=True):
                    ...
                    break
                impactful = run_clean_day(runtime, day)
                if progress_callback:
                    progress_callback(day + 1, max_days)
                final_day = day + 1
                _check_clean_core_daily_invariants(runtime, day)
                console.print(
                    f"[dim]Day {day}: {'activity' if impactful else 'quiet'}[/dim]"
                )
                if update_runtime_stability(
                    runtime,
                    day=day,
                    impactful=impactful,
            ...
            result = run_runtime_until_stable(
                ...
                day_callback=_check_clean_core_daily_invariants,
            )
            finalize_banking_marker_events(
                result.state,
    ...
    if export.get("balances_csv"):
        ...
        write_clean_balances_csv(result, export_path)
    ...
    if export.get("events_jsonl"):
        ...
        write_clean_events_jsonl(result, export_path)
        console.print(f"[green]OK[/green] Exported events to {export_path}")

    if result.state.dealer_metrics is not None:
        ...
        if dealer_metrics_path:
            ...
            metrics = result.state.dealer_metrics
            dealer_summary = dealer_metrics_summary(result.state)
            ...
            if detailed_dealer_logging and not result.state.dealer_config.balanced_passive:
