...
from .models import AgentBalanceView, DayEventsView, DaySummaryView, EventView


def _format_currency(amount: int | float | Decimal, show_sign: bool = False) -> str:
...
def build_events_panel(day_events_view: DayEventsView) -> Panel:
    ...
    return Panel(tree, title=f"Day {day_events_view.day} Events", border_style="blue")
...
def build_day_summary(view: DaySummaryView) -> list[RenderableType]:
    ...
    # Day header
    renderables.append(
        Panel(Text(f"Day {view.day} Summary", style="bold cyan"), border_style="cyan")
    )
