...
def display_events_table_renderable(events: list[dict[str, Any]]) -> RenderableType:
    ...
    if not events:
        return (
            Text("No events to display.", style="dim")
            if RICH_AVAILABLE
            else "No events to display."
        )
...
def _display_events_summary(events: list[dict[str, Any]], console: Console | None = None) -> None:
    ...
    for event in events:
        ...
        if kind == "PayableSettled":
            _print(
                f"Day {day}: 💰 {event['debtor']} → {event['creditor']}: ${event['amount']}",
                console,
            )
        elif kind == "DeliveryObligationSettled":
            qty = event.get("qty", event.get("quantity", "N/A"))
            _print(
                f"Day {day}: 📦 {event['debtor']} → {event['creditor']}: {qty} {event.get('sku', 'items')}",
                console,
            )
        elif kind == "StockTransferred":
            _print(
                f"Day {day}: 🚚 {event['frm']} → {event['to']}: {event['qty']} {event['sku']}",
                console,
            )
...
def _display_events_detailed(events: list[dict[str, Any]], console: Console | None = None) -> None:
...
def _display_day_events(day_events: list[dict[str, Any]], console: Console | None = None) -> None:
...
def _display_single_event(
    event: dict[str, Any], console: Console | None = None, indent: str = "  "
) -> None:
    ...
    if kind == "StockCreated":
        _print(
            f"{indent}🏭 Stock created: {event['owner']} gets {event['qty']} {event['sku']}",
            console,
        )
        ...
        _print(
            f"{indent}✅ Payment settled: {event['debtor']} → {event['creditor']}: ${event['amount']}",
            console,
        )
        ...
        _print(
            f"{indent}✅ Delivery settled: {event['debtor']} → {event['creditor']}: {qty} {sku}",
            console,
        )
        ...
        _print(
            f"{indent}📦 Stock transferred: {event['frm']} → {event['to']}: {event['qty']} {event['sku']}",
            console,
        )

    elif kind == "CashTransferred":
        _print(
            f"{indent}💵 Cash transferred: {event['frm']} → {event['to']}: ${event['amount']}",
            console,
        )

    elif kind == "StockSplit":
        sku = event.get("sku", "N/A")
        event.get("original_qty", 0)
        ...
        _print(
            f"{indent}📊 Stock split: {short_orig} → {short_new}: {split_qty} {sku} (keeping {remaining_qty})",
            console,
        )
        ...
        _print(
            f"{indent}📝 Delivery obligation created: {frm} → {to}: {qty} {sku} (due day {due_day})",
            console,
        )
        ...
        _print(
            f"{indent}💳 Client payment: {payer} ({payer_bank}) → {payee} ({payee_bank}): ${amount}",
            console,
        )
...
def display_events_renderable(
    events: list[dict[str, Any]], format: str = "detailed"
) -> list[RenderableType]:
    """
