...
def clean_core_auto_fallback_reason(
    ...
    if default_handling and config.run.default_handling != default_handling:
        config = config.model_copy(
            update={
                "run": config.run.model_copy(update={"default_handling": default_handling})
            }
        )
...
def build_clean_core_banking_config(path: Path) -> Any | None:
    ...
    balanced_cfg = (
        raw_scenario.get("_balanced_config", {})
        if isinstance(raw_scenario.get("_balanced_config"), dict)
        else {}
    )
    ...
    return CleanBankingConfig(
        ...
        enable_bank_lending=bool(
            ...
            or (
                raw_scenario.get("run", {}).get("enable_bank_lending", False)
                if isinstance(raw_scenario.get("run"), dict)
                else False
            )
        ),
        ...
        cb_lending_cutoff_day=(
            int(balanced_cfg["cb_lending_cutoff_day"])
            if balanced_cfg.get("cb_lending_cutoff_day") is not None
            else None
        ),
        trader_bank_assignments={
            str(agent_id): [str(bank_id) for bank_id in bank_ids]
            for agent_id, bank_ids in (
                balanced_cfg.get("trader_bank_assignments", {}) or {}
            ).items()
        },
        infra_bank_assignments={
            str(agent_id): str(bank_id)
            for agent_id, bank_id in (
                balanced_cfg.get("infra_bank_assignments", {}) or {}
            ).items()
        },
    )
