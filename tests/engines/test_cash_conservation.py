...
def total_system_cash(system: System) -> Decimal:
    ...
    Iterates over each agent's asset_ids and accumulates the amount of
    every contract whose kind is ``InstrumentKind.CASH``.

    Returns:
        The aggregate cash balance as a Decimal.
    """
    total = Decimal(0)
